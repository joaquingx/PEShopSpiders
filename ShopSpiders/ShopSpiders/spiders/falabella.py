...
class FalabellaSpider(scrapy.Spider):
    name = 'falabella'
    allowed_domains = ['www.falabella.com.pe/falabella-pe']
    start_urls = ['http://www.falabella.com.pe/falabella-pe']
    ...
    def parse_categorie(self, response):
        if len(response.xpath('//head/link[@rel="next"]/@href').extract()):
            ...
            yield response.follow(url_url_final, callback=self.parse_categorie_json, meta={'categorie': nav_state}, dont_filter=True)
    ...
    def parse_categorie_json(self, response):
        ...
        if json_response['success']:
            ...
            for i in range(1, json_response['pagesTotal']+1):
                ...
                url_encode = quote_plus(url_tab)
                url_url_final='https://www.falabella.com.pe/rest/model/falabella/rest/browse/BrowseActor/get-product-record-list?'+url_encode
                yield response.follow(url_url_final, callback=self.parse_categorie_item_list, meta={'categorie': nav_state,'page_number':i}, dont_filter=True)
        else:
